...
import biz.enon.tra.uae.rest.robo_requests.ChangeUserProfileRequest;
...
public final class EditUserProfileFragment extends BaseFragment
        implements OnClickListener, ProfileController.OnControllerButtonClickListener, AttachmentManager.OnImageGetCallback,
        OnOpenPermissionExplanationDialogListener, OnImageSourceSelectListener, AlertDialogFragment.OnOkListener, OnCheckedChangeListener {

    public static final String KEY_USER_PROFILE_MODEL = "USER_PROFILE_MODEL";
    ...
    private void initUserAvatar(final UserProfileResponseModel _userProfile) {
        if (_userProfile.getImageUrl().isEmpty()) {
            hvUserAvatar.postScaleType(HexagonView.ScaleType.INSIDE_CROP);
    ...
    public void onAttachmentGet(@NonNull final Uri _imageUri) {
        mImageUri = _imageUri;
        Picasso.with(getActivity()).load(mImageUri).into(new HexagonViewTarget(hvUserAvatar));
    }

    @Override
    public void onControllerButtonClick(final View _view, final @ProfileController.ControllerButton int _buttonId) {
        switch (_buttonId) {
