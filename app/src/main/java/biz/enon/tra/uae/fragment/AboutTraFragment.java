...
import biz.enon.tra.uae.rest.robo_requests.ContactUsRequest;
...
public class AboutTraFragment extends BaseFragment implements OnTabSelectedListener {//}, LoaderOverlayController {
    ...
    private ContactUsResponse.List mContactUsData;
    ...
    public void onStart() {
        super.onStart();
        getSpiceManager().getFromCache(ContactUsResponse.List.class, KEY_CONTACT_US, DurationInMillis.ALWAYS_RETURNED, mRequestListener);
    }
    ...
    private final class ContactUsInfoResponseListener implements RequestListener<ContactUsResponse.List>, Loader.Cancelled {

        @Override
        public void onRequestSuccess(final @Nullable ContactUsResponse.List result) {
            getSpiceManager().removeDataFromCache(ContactUsResponse.List.class, KEY_CONTACT_US);
            if (isAdded()) {
